...
## 7G. Completed: Git version control setup

This repo's files existed on disk and were already live on GitHub Pages, but the local working copy was never under git — no `.git` directory existed here before this session, despite `https://github.com/monagib3/delivery-app` already having its own commit history.

- **Local `git init`**, remote `origin` set to `https://github.com/monagib3/delivery-app.git`, local `main` tracks `origin/main`.
- **Reconciliation:** local disk state (identical to remote for most files, but ahead on `index.html`/`returns.html`/`inventory-ops.html`/`CLAUDE.md`/`DOCUMENTATION.md` — see §7F and doc updates made before git existed locally) was committed as a baseline, then merged with `origin/main --allow-unrelated-histories`. This produced 3 add/add conflicts (one `.home-btn` CSS block + one `<a>` tag per file), resolved in favor of the local content, verified as zero-diff against the pre-merge local commit before finalizing the merge commit.
- **`.gitignore`** added, excluding `.claude/settings.local.json`. That file was already committed in the initial baseline commit *before* `.gitignore` existed, so it remains tracked in history — `.gitignore` only prevents future changes to it from being picked up. Fully untracking it would need an explicit `git rm --cached .claude/settings.local.json` + commit, not yet done.
- **Push workflow:** commits pushed directly to `main`, no branch/PR review step in use — matches this being a single-owner internal tool.

**Feature complete.**

---

## 7H. Completed: Business Manager Dashboard

Read-only reporting page for the business manager — stock levels, per-distributor sales, and pending orders in one view, without recomputing anything already computed by the formula-driven reporting sheets.

- **`DashboardBackend.js`** (new) — `getDashboardData()` (opens `ss` via `SPREADSHEET_ID`) → `_getDashboardData(ss)`, composed of three read-only helpers:
  - `_readStockByFlavor(ss)` — reads `🌿 Per-Flavor Breakdown` cols A/F/G (Flavor, Current Stock, Below Reorder Threshold) directly. No SUMPRODUCT/formula logic in Apps Script — cell values only.
  - `_readSalesByDistributor(ss)` — reads `📈 Per-Distributor Summary` cols A/C/D/E (Distributor, Cases Ordered, Cases Delivered, Fill Rate %) directly. Same rule — read, don't recompute.
  - `_readPendingOrders(ss)` — extends `_getOpenOrders(ss)` from `DeliveryBackend.js` (reused as-is, not duplicated) with cases outstanding (delivered qty summed from `🚚 Delivery_Lines` per Order_ID, subtracted from total ordered) and days open (via `_daysOpen()`). Includes both Pending and Partial status (same filter `_getOpenOrders` already applies), sorted newest first.
- **`_daysOpen(dateStr)`** — timezone-consistent date-only diff. Formats "today" via `Session.getScriptTimeZone()` (same tz `_getOpenOrders` uses for `order.date`) and compares both dates as `Date.UTC()` midnight representations of their calendar date, rather than subtracting `Date` instants directly — avoids an off-by-one that naive `new Date(dateStr)` arithmetic would introduce across timezones.
- **No try/catch in `DashboardBackend.js`** — deliberate, matches the existing GET-read convention: `getOpenOrders()` and the `_getConfig()` calls backing `getReturnContext`/`getProductionContext`/`getDistributors` don't wrap themselves either. Only mutating POST actions (`submitDelivery`, `submitOrder`, etc.) use try/catch, since `Router.js`'s `doPost` has the outer try/catch and `doGet` doesn't.
- **Router additions:** `doGet ?action=getDashboardData` — same shape as `getReturnContext`/`getProductionContext`/`getDistributors`: opens `ss`, calls `_getDashboardData(ss)` directly, returns `{ success: true, ...data }`. Routing-table comment updated.
- **Frontend:** `dashboard.html`, GitHub Pages only. Reuses `returns.html`'s palette, header, `.home-btn`, loading-overlay, and error-msg blocks verbatim; adds one new `.dash-table` component. Same `SCRIPT_URL` deployment endpoint as the other internal frontends. Three read-only sections (Stock by Flavor, Sales by Distributor, Pending Orders), single `fetch` on `window.onload`, no forms/POST calls. `belowThreshold` rendering handles all three sheet values (`YES`/`NO`/`—`).
- **`home.html`:** new 5th launcher card (📊 لوحة التحكم / Dashboard) → `dashboard.html`, `animation-delay:0.25s`, same markup pattern as the existing 4 cards.
- **Deployment:** deployed as a new Apps Script version (Manage Deployments → New version), `dashboard.html`/`home.html` pushed to GitHub Pages, verified live end-to-end on device.

**Feature complete.**

---

## 7I. In progress — backend/Router complete, frontend 4-of-5 done: Production Lifecycle (PT1–PT7, partial)

Production Lifecycle domain — request → production run → QC decision → inventory pull — tracked in its own spreadsheet, independent of the Order Tracker and Inventory Tracker. **Schema (PT1), all four backend functions (PT2–PT5), and Router wiring (PT6) are complete and live-verified.** Frontend (PT7) is 4 of 5 pieces done: `production-request.html`, `production-run.html`, `qc-decision.html`, and the PT7.4 batch-pull addition to `inventory-ops.html` are all built, deployed to GitHub Pages, and functionally verified end-to-end in a live browser. **Still missing: the `home.html` launcher updates (PT7.5) — none of the built pages are reachable from the home screen yet, only by direct URL — and the final cross-page browser test (PT7.6).** Do not treat this as a finished, team-usable feature until PT7 is fully done (see the session handoff below for exact next steps).

- **New spreadsheet:** "Badr El-Din Production Tracker", find-or-create via `_createOrGetProductionSpreadsheet()`/`_getProductionSpreadsheet()` (`ProductionTracker.js`), same pattern as `Inventory.js`. ID stored in Script Properties as `PRODUCTION_SPREADSHEET_ID`.
- **Five sheets**, all built via `ProductionTracker.js` builders, styled to match existing header/zebra/frozen-row conventions:
  ...
  | 🏭 Production_Runs | Run_ID, Request_ID, Date, Lot_Number, Total_Produced, QC_Status, Notes, Inventory_Pulled_By, Inventory_Pulled_Timestamp | Gold `#BF9000`. QC_Status dropdown: Pending / Approved / Rejected. Last two columns added in PT5. |
  | 🏭 Production_Run_Lines | Line_ID, Run_ID, Flavor_ID, Flavor_Name, Qty_Produced | Gold `#BF9000` (matches header sheet) |
  | 🏭 QC_Records | QC_ID, Run_ID, Timestamp, QC_By, Decision, Notes | Purple `#674EA7`. Decision dropdown: Pending / Approved / Rejected — deliberately the same vocabulary as `Production_Runs.QC_Status`, since Decision is the per-check mirror of it. |
- **Five counters**, all now wired to a live caller: `PRQ_COUNTER`/`PRQL_COUNTER` (`ProductionRequestBackend.js`), `PRUN_COUNTER`/`PRUNL_COUNTER` (`ProductionRunBackend.js`), `QC_COUNTER` (`QualityBackend.js`).
- **`Team_Contacts` `Role` column** (6th column, main spreadsheet, added in PT1) is now actually read: `submitProductionRequest`/`submitProductionRun`/`submitQCDecision` each filter active contacts to a specific role (`"Production Engineer"`, `"Quality Engineer"`, `"Inventory"` respectively) OR blank Role, fail-open by design — an unset Role notifies on everything until roles are actually assigned.
- **PT2 `ProductionRequestBackend.js`** — `submitProductionRequest(payload)` writes `Production_Requests`/`Production_Request_Lines` (Status starts `"Requested"`), Flavor_ID/Flavor_Name sourced from the main spreadsheet's `🌿 Flavors` via the standard runtime lookup, notifies Production Engineers.
- **PT3 `ProductionRunBackend.js`** — `submitProductionRun(payload)` validates `requestId` as a real FK (hard error, nothing written on failure), writes `Production_Runs`/`Production_Run_Lines` (`QC_Status` starts `"Pending"`), flips the request's Status to `"Fulfilled"`, allows produced quantities to vary from requested without blocking, notifies Quality Engineers. **Hardened post-PT6** (deployed as Version 21): also rejects before writing anything if the matched request's Status is not `"Requested"` — blocks a second run from being logged against an already-`Fulfilled` request (stale dropdown, concurrent submissions). Verified via the `production-run.html` flow (the dropdown correctly stopped offering a fulfilled request after one run was logged against it) — the guard itself has **not** been separately force-tested with a stale/bypassed request ID (e.g. calling the function directly with a known-Fulfilled `requestId`).
- **PT4 `QualityBackend.js`** — `submitQCDecision(payload)` validates `runId` as a real FK and enforces a symmetric one-way lock (`QC_Status` must be `"Pending"`; once Approved or Rejected, no re-deciding in either direction), writes `QC_Records`, flips the run's `QC_Status`, notifies Inventory contacts with differently-framed messages for Approved (ready for warehouse) vs. Rejected (needs manual write-off).
- **PT5 `InventoryPullBackend.js`** — `submitInventoryPull(payload)` validates `runId` is Approved and not already pulled, pre-fills quantities from `Production_Run_Lines`, and calls the **existing** `submitProduction()` (`ProductionBackend.js`) as-is — no reimplementation of the `🌱 Productions`/`Production_Lines` write, team notification, or Inventory movement logging. `productionDate` is passed through from the run's actual `Date` (not the pull date) so the Inventory ledger reflects when the batch was actually made. Traceability back to the run/request is carried in `submitProduction()`'s `notes` field, since `🌱 Productions` has no FK column into this lifecycle.
- **PT6 `Router.js` wiring** — 4 POST routes (`submitProductionRequest`, `submitProductionRun`, `submitQCDecision`, `submitInventoryPull`) calling PT2–PT5's functions as-is, plus 3 GET routes (`getPendingProductionRequests`, `getPendingQCRuns`, `getPendingInventoryPulls`) for PT7's per-role queues. The three GET routes' actual sheet-read/join logic lives in **`ProductionTracker.js`** (`_getPendingProductionRequests()`/`_getPendingQCRuns()`/`_getPendingInventoryPulls()`), not in `Router.js` itself — deliberate, to keep `Router.js` a pure dispatcher per §2's hard "zero business logic in Router" rule, since `ProductionTracker.js` isn't one of the four PT2–PT5 backend files. Deployed as Version 20; `getPendingProductionRequests` confirmed live via the deployed web app URL.
- **All temporary setup/test/cleanup wrappers used across PT1–PT5 were run once and deleted** per project convention — none remain in source.
- **PT7 frontend progress (4 of 5 done, all green palette per §7D's audience-based palette rule — these are internal-team pages, not distributor-facing like `order.html`):**
  - ✅ `production-request.html` — mirrors `returns.html`'s form-only pattern (Requested By free text instead of a distributor dropdown), reuses the existing `getProductionContext` GET route rather than adding a new one, posts `submitProductionRequest`. Live, tested end-to-end.
  - ✅ `production-run.html` — mirrors `index.html`'s dropdown+badge+flavor-grid pattern, reads `getPendingProductionRequests`, adds a Date/Lot Number "Run Details" card, posts `submitProductionRun`. Live, tested end-to-end.
  - ✅ `qc-decision.html` — mirrors `index.html`'s dropdown+badge pattern but with a **read-only** produced-quantities display (non-zero flavors only, no steppers — QC reviews, doesn't re-enter numbers) and an Approve/Reject toggle-button pair instead of a dropdown, reads `getPendingQCRuns`, posts `submitQCDecision`. Live, tested end-to-end.
  - ✅ **PT7.4 — inventory pull, built as a mode inside `inventory-ops.html` instead of a standalone `inventory-pull.html`.** Design change from the original PT7 plan, approved before implementation. Production mode gained a "Manual Entry" / "From Approved Batch" sub-toggle: Manual Entry is the pre-existing Production In flow, byte-for-byte unchanged; "From Approved Batch" reads `getPendingInventoryPulls`, offers a dropdown of Approved-and-not-yet-pulled runs, and on selection pre-fills the existing flavor grid **read-only** from that run's quantities (steppers hidden, inputs disabled) — reusing the grid markup rather than building a second one. The Notes field is replaced by a required "Pulled By" field, and submission posts `submitInventoryPull({runId, pulledBy})`. No new Backend/Router code was needed — both were already wired in PT5/PT6. **Quantities are deliberately locked in this mode** — `submitInventoryPull` doesn't accept a quantity override, and the numbers already passed QC — this is a design decision, not a limitation to fix later. Write-off mode's fields and dispatch logic are untouched. Deployed via `git push` only (GitHub Pages); no `clasp push`/new Apps Script version was needed since no backend files changed. Live, tested end-to-end: `PRUN-002` and a second run were both pulled successfully, WhatsApp notifications were received, and the resulting ` Inventory Movements` rows were confirmed correct.
  - ❌ `home.html` launcher updates — **not yet done (PT7.5).** `production-request.html`, `production-run.html`, and `qc-decision.html` are still reachable only by direct URL, not linked from the home screen. Per the approved plan: 3 new green-palette cards (🏭 Production Request, ⚙️ Log Production Run, 🔬 QC Review), continuing the staggered `animation-delay` sequence — the existing Inventory Ops card already covers the new batch-pull mode, since it lives inside `inventory-ops.html`.
  - Deliberately **not** factored into a shared CSS/JS file across the pages — matches the unbroken existing project convention that every frontend is fully self-contained (confirmed decision, see PT7 planning discussion).
- **WhatsApp/CallMeBot delivery issue — resolved.** The QC-Approve notification that didn't arrive during the prior session's testing was confirmed to be a CallMeBot quota/rate-limit issue, not a code bug — no fix was needed. Consistent with the pattern already seen during PT3 testing (2 messages 2 seconds apart, only 1 arrived, both returned HTTP 200). Notifications during PT7.4 testing (batch-pull `submitInventoryPull` → `submitProduction` → `_notifyProduction`) were received successfully.
- **Live test data, deliberately NOT cleaned up.** `PRQ-001`/`PRQ-002` and their production requests/runs/QC records remain in the Production Tracker spreadsheet, including the two runs pulled during PT7.4 testing (one of them `PRUN-002`). None of the five counters (`PRQ_COUNTER`/`PRQL_COUNTER`/`PRUN_COUNTER`/`PRUNL_COUNTER`/`QC_COUNTER`) have been reset. Cleanup is intentionally deferred until PT7.5/PT7.6 are done — see the plan below.
- **Next session plan, in order:**
  1. Build `home.html` launcher updates (PT7.5) — 3 new cards (Production Request, Production Run, QC Review); the existing Inventory Ops card already covers batch pull.
  2. Final end-to-end browser test across all pages together (PT7.6).
  3. **Only then:** combined cleanup — delete all live Production Lifecycle test rows and their line rows across the 5 Production Tracker sheets, reset all 5 counters to 0.
  4. Final `CLAUDE.md`/`DOCUMENTATION.md` update marking the full Production Lifecycle feature complete.

**Status: backend + Router complete and live-verified (through `submitProductionRun`'s post-PT6 hardening, Version 21). Frontend 4 of 5 built and live-verified — PT7.1–PT7.4 done (`production-request.html`, `production-run.html`, `qc-decision.html`, and `inventory-ops.html`'s batch-pull mode); `home.html` launcher updates (PT7.5) and the final cross-page test (PT7.6) remaining. No open issues.**
