...
## 7I. In progress — backend/Router complete, frontend 3-of-5 done: Production Lifecycle (PT1–PT7, partial)

Production Lifecycle domain — request → production run → QC decision → inventory pull — tracked in its own spreadsheet, independent of the Order Tracker and Inventory Tracker. **Schema (PT1), all four backend functions (PT2–PT5), and Router wiring (PT6) are complete and live-verified.** Frontend (PT7) is 3 of 5 pieces done: `production-request.html`, `production-run.html`, and `qc-decision.html` are built, deployed to GitHub Pages, and functionally verified end-to-end in a live browser. **Still missing: `inventory-pull.html` and the `home.html` launcher updates — none of the four built pages are reachable from the home screen yet, only by direct URL.** Do not treat this as a finished, team-usable feature until PT7 is fully done (see the session handoff below for exact next steps).
...
- **PT7 frontend progress (3 of 5 done, all green palette per §7D's audience-based palette rule — these are internal-team pages, not distributor-facing like `order.html`):**
  ...
  - ❌ `inventory-pull.html` — **not yet built.** Per the approved combined PT7 plan: mirrors `index.html`'s dropdown+badge pattern (simplest of the four — payload is just `{runId, pulledBy}`, everything else server-side pre-filled), reads `getPendingInventoryPulls`, posts `submitInventoryPull`.
  - ❌ `home.html` launcher updates — **not yet done.** All four built pages are currently reachable only by direct URL, not linked from the home screen. Per the approved plan: 4 new green-palette cards (🏭 Production Request, ⚙️ Log Production Run, 🔬 QC Review, 📥 Inventory Pull), continuing the staggered `animation-delay` sequence.
  - Deliberately **not** factored into a shared CSS/JS file across the four pages — matches the unbroken existing project convention that every frontend is fully self-contained (confirmed decision, see PT7 planning discussion).
- **OPEN ISSUE, unresolved — investigate first thing next session:** the WhatsApp notification from the QC Approve step (`qc-decision.html` → `submitQCDecision` → `_notifyQCDecision`) did not arrive on the test phone, despite `Team_Contacts` showing one correctly-configured active contact (`Is_Active=TRUE`, `Role` blank — should fail-open match). Leading hypothesis, **not confirmed**: CallMeBot's informal/undocumented rate-limiting silently dropping messages sent to the same number in quick succession — same pattern observed (and accepted, not investigated) during PT3 testing, where 2 messages 2 seconds apart only had 1 arrive despite both returning HTTP 200. A standalone diagnostic function `_testWhatsAppOnly()` (isolates the WhatsApp send from the rest of the QC flow) was proposed but **never added or run**. Do this before trusting notification delivery further in any Production Lifecycle testing.
- **Live test data, deliberately NOT cleaned up.** Both `PRQ-001` and `PRQ-002` were created and tested through `production-run.html`, and at least one resulting run was carried through QC (Approved). **Exact IDs are not confirmed here** — the session ended without a final verification pass, so don't trust specific IDs from memory. **First thing next session:** open `🏭 Production_Requests`, `🏭 Production_Runs`, and `🏭 QC_Records` directly in the spreadsheet and confirm exactly which rows exist, each run's `QC_Status`/`Inventory_Pulled_By`, and which one run is `QC_Status = Approved` AND `Inventory_Pulled_By` still blank (that's the one to test `inventory-pull.html` against) — before doing anything else. None of the five counters (`PRQ_COUNTER`/`PRQL_COUNTER`/`PRUN_COUNTER`/`PRUNL_COUNTER`/`QC_COUNTER`) have been reset — do not reset them until the combined cleanup pass below.
- **Next session plan, in order:**
  1. Open the sheets directly and confirm the exact live test data (IDs, statuses, which run is Approved-and-not-pulled) — see above.
  2. Add and run `_testWhatsAppOnly()` to isolate and confirm/deny the WhatsApp delivery issue above.
  3. Build and test `inventory-pull.html` (PT7.4) against the confirmed Approved-not-yet-pulled run.
  4. Build `home.html` launcher updates (PT7.5) — 4 new cards for all PT7 pages.
  5. Final end-to-end browser test across all 4 pages together (PT7.6).
  6. **Only then:** combined cleanup — delete all live Production Lifecycle test rows and their line rows across the 5 Production Tracker sheets, reset all 5 counters to 0.
  7. Final `CLAUDE.md`/`DOCUMENTATION.md` update marking the full Production Lifecycle feature complete.

**Status: backend + Router complete and live-verified (through `submitProductionRun`'s post-PT6 hardening, Version 21). Frontend 3 of 5 built and live-verified; `inventory-pull.html` and `home.html` updates remaining. One unresolved WhatsApp delivery open issue — see above.**
