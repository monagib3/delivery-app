...
## 7I. In progress — schema only: Production Lifecycle (PT1)

Foundation for a new Production Lifecycle domain — request → production run → QC — tracked in its own spreadsheet, independent of the Order Tracker and Inventory Tracker. **Sub-task PT1 is schema only: no backend functions, Router routes, or frontend exist yet.** Do not treat this as a working feature.

- **New spreadsheet:** "Badr El-Din Production Tracker", find-or-create via `_createOrGetProductionSpreadsheet()`/`_getProductionSpreadsheet()` (new file `ProductionTracker.js`), same pattern as `Inventory.js`. ID stored in Script Properties as `PRODUCTION_SPREADSHEET_ID`.
...
- **Five new counters** added to the `Code.js` counter registry (documentation-only comment block): `PRQ_COUNTER` ("PRQ-"), `PRQL_COUNTER` ("PRQL-"), `PRUN_COUNTER` ("PRUN-"), `PRUNL_COUNTER` ("PRUNL-"), `QC_COUNTER` ("QC-"). Marked **not yet wired to a caller**, same status as `OL_COUNTER`/`DL_COUNTER`/`RL_COUNTER`/`PL_COUNTER` were before their respective backends were built. No Script Properties seeding was needed — there's no historical data, and `_nextSequentialId()` already defaults an unset property to 0, so the first ID generated (once a backend calls it in a later phase) will naturally be `-001`.
- **`Team_Contacts` gained a `Role` column** (6th column, main spreadsheet) — header only, **not yet populated**, **not yet read by any code**. `_buildTeamContactsSheet` (Code.js) now writes it on any future full rebuild. The live sheet was updated surgically via a temporary wrapper that did not clear existing rows, so pre-existing Phone/CallMeBot_ApiKey/Name data was left untouched.
- **Setup wrappers used and deleted** (per project convention — temporary, run once via the Apps Script editor, then removed from source): `_runBuildProductionTracker()` (ProductionTracker.js) built the 5 sheets; `_addRoleColumnToTeamContacts()` (Code.js) added the Role header to the live sheet. Both confirmed run clean and deleted.
- **Explicitly not built yet:** no `ProductionTrackerBackend.js`-style backend file, no Router actions, no frontend. Flavor_ID/Flavor_Name on `Production_Request_Lines`/`Production_Run_Lines` must be sourced from `🌿 Flavors` via the same runtime `Flavor_Name → Flavor_ID` lookup pattern already used by `Order_Lines`/`Delivery_Lines`/`Return_Lines`/`Production_Lines` — not a separate or hardcoded flavor list — when that backend work happens.

**Status: schema only, not feature complete.**
