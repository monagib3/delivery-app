...
## 6B. Production Tracker Spreadsheet — "Badr El-Din Production Tracker" (complete)

**Status: complete.** Schema, all four backend functions, and Router wiring are live-verified. Frontend (PT7) is fully built — `production-request.html`, `production-run.html`, `qc-decision.html`, the PT7.4 batch-pull mode added to `inventory-ops.html`, and the PT7.5 `home.html` launcher cards linking all three new pages are deployed to GitHub Pages and functionally verified end-to-end in a live browser, including the final PT7.6 cross-page test. Third spreadsheet, separate from the Order Tracker and Inventory Tracker, for the Production Lifecycle domain (request → production run → QC decision → inventory pull). Created/accessed via `_createOrGetProductionSpreadsheet()`/`_getProductionSpreadsheet()` in `ProductionTracker.js`, ID stored in Script Properties as `PRODUCTION_SPREADSHEET_ID`.
...
- ✅ **`home.html` launcher updates (PT7.5).** `production-request.html`, `production-run.html`, and `qc-decision.html` are now linked from the home screen via 3 new green-palette cards (🏭 Production Request, ⚙️ Log Production Run, 🔬 QC Review) — the existing Inventory Ops card already covers the batch-pull mode. Live, tested end-to-end.
...
| **Production Lifecycle — PT1–PT7** | Complete — request → production run → QC decision → inventory pull, tracked in its own "Badr El-Din Production Tracker" spreadsheet (`PRODUCTION_SPREADSHEET_ID`) with 5 sheets, built via `ProductionTracker.js`. All 5 counters wired to live callers. `Team_Contacts` `Role` column read for role-filtered WhatsApp notifications. Four backend files (`ProductionRequestBackend.js`, `ProductionRunBackend.js` — hardened post-PT6 with a Status guard against re-running an already-Fulfilled request, deployed Version 21 — `QualityBackend.js`, `InventoryPullBackend.js`) plus `Router.js` wiring. The inventory pull flow was added as a "From Approved Batch" sub-mode inside `inventory-ops.html`'s existing Production mode rather than a standalone page — read-only, pre-filled quantities (locked by design, since `submitInventoryPull` doesn't accept a quantity override and the numbers already passed QC), posting `submitInventoryPull({runId, pulledBy})`. `home.html` gained 3 launcher cards (Production Request, Production Run, QC Review) linking the remaining frontend pages (PT7.5). Final cross-page browser test (PT7.6) passed clean, no open issues. Live test data cleaned up manually — all 5 Production Tracker sheets cleared, `PRQ_COUNTER`/`PRQL_COUNTER`/`PRUN_COUNTER`/`PRUNL_COUNTER`/`QC_COUNTER` reset to 0; test rows removed from `🌱 Productions`/`Production_Lines` and ` Inventory Movements`, `PRD_COUNTER`/`PL_COUNTER` rolled back to 4/12. See `CLAUDE.md` §7I and DOCUMENTATION.md §6B. |
