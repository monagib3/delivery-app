...
| `ProductionTracker.js` | Production Tracker spreadsheet creation/access (`_createOrGetProductionSpreadsheet`, `_getProductionSpreadsheet`), the 5 sheet builders for the Production Lifecycle domain (`_buildProductionRequestsSheet`, `_buildProductionRequestLinesSheet`, `_buildProductionRunsSheet`, `_buildProductionRunLinesSheet`, `_buildQCRecordsSheet`), and the three PT6 pending-queue reads (`_getPendingProductionRequests`, `_getPendingQCRuns`, `_getPendingInventoryPulls`) called by `Router.js`'s GET routes — kept here rather than in `Router.js` itself so the router stays a pure dispatcher. See §6B. |
...
## 6B. Production Tracker Spreadsheet — "Badr El-Din Production Tracker" (backend/Router complete, frontend 4-of-5 done)

**Status: schema, all four backend functions, and Router wiring are complete and live-verified. Frontend (PT7) is 4 of 5 pieces done** — `production-request.html`, `production-run.html`, `qc-decision.html`, and the PT7.4 batch-pull mode added to `inventory-ops.html` are all built, deployed to GitHub Pages, and functionally verified end-to-end in a live browser. The `home.html` launcher updates (PT7.5) and the final cross-page browser test (PT7.6) are not yet done — see the session handoff at the end of this section for exact next steps. Third spreadsheet, separate from the Order Tracker and Inventory Tracker, for the Production Lifecycle domain (request → production run → QC decision → inventory pull). Created/accessed via `_createOrGetProductionSpreadsheet()`/`_getProductionSpreadsheet()` in `ProductionTracker.js`, ID stored in Script Properties as `PRODUCTION_SPREADSHEET_ID`.
...
**`Team_Contacts` (main spreadsheet) `Role` column** (6th column, added in this domain's PT1) is now actually read by `submitProductionRequest`/`submitProductionRun`/`submitQCDecision`, each filtering active contacts to a specific role (`"Production Engineer"`, `"Quality Engineer"`, `"Inventory"` respectively) OR blank Role — fail-open by design.

**Backend files:**
- `ProductionRequestBackend.js` — `submitProductionRequest(payload)`: writes `Production_Requests`/`Production_Request_Lines` (Status starts `"Requested"`), sources Flavor_ID/Flavor_Name from the main spreadsheet's `🌿 Flavors` via the standard runtime lookup, notifies Production Engineers.
- `ProductionRunBackend.js` — `submitProductionRun(payload)`: validates `requestId` as a real FK (hard error before any write on failure), writes `Production_Runs`/`Production_Run_Lines` (`QC_Status` starts `"Pending"`), flips the request's Status to `"Fulfilled"`, allows produced quantities to vary from requested without blocking, notifies Quality Engineers. Hardened after PT6 (deployed as Version 21): also rejects before writing anything if the matched request's Status is not `"Requested"`, blocking a second run against an already-`Fulfilled` request. Verified via the `production-run.html` flow; not separately force-tested with a stale/bypassed request ID.
- `QualityBackend.js` — `submitQCDecision(payload)`: validates `runId` as a real FK and enforces a symmetric one-way lock (`QC_Status` must be `"Pending"`; once Approved or Rejected, no re-deciding either direction), writes `QC_Records`, flips the run's `QC_Status`, notifies Inventory contacts with differently-framed messages for Approved (ready for warehouse) vs. Rejected (needs manual write-off).
- `InventoryPullBackend.js` — `submitInventoryPull(payload)`: validates the run is Approved and not already pulled, pre-fills quantities from `Production_Run_Lines`, and calls the **existing** `submitProduction()` (`ProductionBackend.js`) as-is — no reimplementation of the `🌱 Productions`/`Production_Lines` write, team notification, or Inventory movement logging. `productionDate` is passed through from the run's actual `Date`, not the pull date, so the Inventory ledger reflects when the batch was actually made. Traceability is carried in `submitProduction()`'s `notes` field, since `🌱 Productions` has no FK column into this lifecycle.

**Router.js wiring:** 4 POST routes (`submitProductionRequest`, `submitProductionRun`, `submitQCDecision`, `submitInventoryPull`) calling the backends above as-is, plus 3 GET routes (`getPendingProductionRequests`, `getPendingQCRuns`, `getPendingInventoryPulls`) for the per-role queues PT7's frontends will need. The GET routes' actual sheet-read/join logic lives in `ProductionTracker.js` (`_getPendingProductionRequests()`/`_getPendingQCRuns()`/`_getPendingInventoryPulls()`), not `Router.js` itself, keeping `Router.js` a pure dispatcher per §8's "Router has zero business logic" convention. Deployed as Version 20; `getPendingProductionRequests` confirmed live via the deployed web app URL.

**PT7 frontend progress (4 of 5 done):** all pages use the green palette (internal-team audience, same reasoning as §7D's `order.html` gold/ink split) and deliberately duplicate their CSS/JS rather than share a file, matching the unbroken existing convention that every frontend in this project is fully self-contained.
- ✅ `production-request.html` — mirrors `returns.html`'s form pattern, reuses the existing `getProductionContext` GET route, posts `submitProductionRequest`. Live, tested end-to-end.
- ✅ `production-run.html` — mirrors `index.html`'s dropdown+badge+flavor-grid pattern, reads `getPendingProductionRequests`, adds Date/Lot Number fields, posts `submitProductionRun`. Live, tested end-to-end.
- ✅ `qc-decision.html` — mirrors `index.html`'s dropdown+badge pattern with a read-only produced-quantities display (non-zero only) instead of steppers and an Approve/Reject toggle-button pair instead of a dropdown, reads `getPendingQCRuns`, posts `submitQCDecision`. Live, tested end-to-end.
- ✅ **PT7.4 — inventory pull**, built as a new mode inside the existing `inventory-ops.html` rather than a standalone `inventory-pull.html` (design change from the original PT7 plan, approved before implementation — no new frontend file, no new Backend/Router code, since `submitInventoryPull`/`getPendingInventoryPulls` were already wired in PT5/PT6). Production mode gained a "Manual Entry" / "From Approved Batch" sub-toggle: Manual Entry is the pre-existing Production In flow, unchanged; "From Approved Batch" reads `getPendingInventoryPulls`, offers a dropdown of Approved-and-not-yet-pulled runs, and on selection pre-fills the existing flavor grid **read-only** (steppers hidden, inputs disabled) from that run's quantities — the grid markup is reused, not duplicated. The Notes field is replaced by a required "Pulled By" field, and submission posts `submitInventoryPull({runId, pulledBy})`. Quantities are **deliberately locked** in this mode — `submitInventoryPull` doesn't accept a quantity override and the numbers already passed QC, so this is a design decision, not a limitation to fix later. Write-off mode is untouched. Deployed via `git push` only (GitHub Pages) — no `clasp push`/new Apps Script version needed, since no backend files changed. Live, tested end-to-end: `PRUN-002` and a second run were both pulled successfully, WhatsApp notifications were received, and the resulting ` Inventory Movements` rows were confirmed correct.
- ❌ `home.html` launcher updates (PT7.5) — not yet done. `production-request.html`, `production-run.html`, and `qc-decision.html` are still reachable only by direct URL. Planned: 3 new green-palette cards (🏭 Production Request, ⚙️ Log Production Run, 🔬 QC Review) — the existing Inventory Ops card already covers the new batch-pull mode.

**WhatsApp/CallMeBot delivery issue — resolved.** The QC-Approve notification that didn't arrive during the prior session's testing was confirmed to be a CallMeBot quota/rate-limit issue, not a code bug — no fix was needed. Consistent with the pattern already seen during PT3 testing (2 messages 2 seconds apart, only 1 arrived, both returned HTTP 200). Notifications during PT7.4 testing were received successfully.

**Live test data, deliberately not cleaned up.** `PRQ-001`/`PRQ-002` and their production requests/runs/QC records remain in the Production Tracker spreadsheet, including the two runs pulled during PT7.4 testing (one of them `PRUN-002`). None of the five Production Lifecycle counters have been reset. Cleanup is intentionally deferred until PT7.5/PT7.6 (below) are done.

**Next-session plan:** (1) build the `home.html` launcher updates (PT7.5) — 3 new cards, since Inventory Ops already covers batch pull; (2) final end-to-end browser test across all pages together (PT7.6); (3) only then, combined cleanup of all live Production Lifecycle test rows and line rows, counters reset to 0; (4) final documentation pass marking the full Production Lifecycle feature complete.
...
| **Production Lifecycle — PT1–PT7 (backend/Router complete, frontend 4-of-5)** | **In progress — backend and Router wiring complete and live-verified; frontend 4 of 5 pieces built, deployed, and live-tested (`production-request.html`, `production-run.html`, `qc-decision.html`, and a PT7.4 batch-pull mode added to `inventory-ops.html`); `home.html` launcher updates (PT7.5) and the final cross-page test (PT7.6) remaining.** New "Badr El-Din Production Tracker" spreadsheet (`PRODUCTION_SPREADSHEET_ID`) with 5 sheets, built via `ProductionTracker.js`. All 5 counters wired to live callers. `Team_Contacts` `Role` column now actually read for role-filtered WhatsApp notifications. Four backend files (`ProductionRequestBackend.js`, `ProductionRunBackend.js` — hardened post-PT6 with a Status guard against re-running an already-Fulfilled request, deployed Version 21 — `QualityBackend.js`, `InventoryPullBackend.js`) plus `Router.js` wiring. **PT7.4 note:** rather than a standalone `inventory-pull.html`, the pull flow was added as a "From Approved Batch" sub-mode inside `inventory-ops.html`'s existing Production mode — read-only, pre-filled quantities (locked by design, since `submitInventoryPull` doesn't accept a quantity override and the numbers already passed QC), posting `submitInventoryPull({runId, pulledBy})`. No backend/Router changes were needed; deployed via `git push` only. **No open issues** — the prior session's QC-Approve WhatsApp notification gap was confirmed as a CallMeBot quota/rate-limit issue, not a code bug. **Live test data intentionally not cleaned up** (`PRQ-001`/`PRQ-002` and their runs/lines, including the two runs — one of them `PRUN-002` — pulled during PT7.4 testing; counters not reset) — cleanup deferred until PT7.5/PT7.6 are done. See `CLAUDE.md` §7I and DOCUMENTATION.md §6B for the full session handoff and next-steps list. |
